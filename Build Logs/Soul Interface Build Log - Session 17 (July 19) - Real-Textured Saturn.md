...
Verified with a Playwright reproduction of Ricky's exact framing (zoom in ~18 scroll-wheel steps, then orbit toward the upper-left crater field) — terrain now reads as coherent, naturally eroded surface with no quilting, and a normal-distance screenshot confirms crater rims and shadow definition are still clearly present, so the bump reduction didn't flatten it out. Bumped cache-busting to `?v=6`. Synced to `Aion/Frontend/Code/`. **Committed** as a follow-up.

---

### Part 33 — The Tiers Didn't *Blend*: Softened the Seams Between Detail Levels (v3)

Ricky's next round of feedback was more specific: "the textures just don't transition well between each other... it needs to be balance[d] out." Looked at the full texture again with that framing rather than just "is it sharp" — and it was right: the crisp encounter-hemisphere crater field cuts directly into a nearly-featureless flat fill at both poles, with almost no gradient between them. Checked the raw 25K source at native resolution to make sure this wasn't something our downsampling introduced — it wasn't; the flat patches have essentially zero detail even at full resolution, because they're regions neither New Horizons nor the 2002 Hubble fill ever resolved anything in. There's no real detail to recover there at any resolution.

What actually needed fixing wasn't the flatness itself (already accurate) — it was the abruptness of the handoff. Rewrote `compose_pluto_texture.py` (v3) to compute a per-pixel "flatness" map from local luminance variance (wide sigma, so it judges regions rather than reacting to single-pixel noise), then:
1. Blend each pixel toward a locally-blurred version of itself, weighted by that flatness map. Detailed regions (flatness ~0) are untouched; already-flat regions were smooth anyway so this is a no-op there too. The actual effect lands on the *ramp* between them — since the flatness map itself is heavily blurred (sigma=60) before use, detail now fades out gradually across a real span of pixels instead of stopping dead at a boundary.
2. Blend in very low-amplitude two-octave grain (coarse soft blobs + fine grain, both smoothed) into the same flatness-weighted regions, so a dead-flat patch reads as soft real terrain rather than an obviously separate pasted-in fill. Amplitude is deliberately low — this is texture grain, not simulated craters; first attempt (Part-33-internal, before landing on this version) used amplitude too low to be visible at all, second pass raised it enough to actually register without inventing any specific geographic feature.

First implementation attempt hit a real bug — PIL's `GaussianBlur` doesn't accept 32-bit-float (`'F'` mode) images on this Pillow build, needed for the wide-range variance math (`gray**2` produces values up to ~65000). Replaced with a hand-rolled cumsum-based box blur (3 passes approximates a true Gaussian, a standard technique) that works on arbitrary float ranges without adding a scipy dependency.

Verified by cropping the actual seam regions (bottom crater-to-pole boundary, top polar band, the streaky right-side transition) at full resolution before and after — the bottom seam in particular went from a hard graphic cutoff to a natural-looking haze-like fade, which is a real, visible difference, not a marginal one. Confirmed on the live sphere too (default view and orbited toward the seam): the soft region now reads as "less-detailed real terrain" rather than "a flat sticker." Bumped cache-busting to `?v=7`. Synced to `Aion/Frontend/Code/`.

---

### Part 34 — A Real New Bug (Bump-Map Rectangle) Plus a Different-in-Kind Fix, Not Another Texture Tweak

Ricky's next screenshot showed a distinct hard-edged rectangular artifact in the shadow/terminator region, and — fairly — called out that the last several passes had all landed the same result: "you haven't done anything... same result the last 4 passes or more." Two separate things needed addressing, and conflating them would have meant another tweak that didn't actually change anything.

**The rectangle was a real, new, fixable bug.** Cropped the plain color texture at that location — clean, no artifact — which meant it was bump-map-only, i.e. only visible once that region's brightness was read as height and lit at a grazing angle. Traced it to Part 33's coarse grain layer: generated on a downsampled `gray.shape[0]//6` grid and upsampled via `Image.BICUBIC` to full resolution. Invisible in flat color, but BICUBIC upsampling of a coarse random grid leaves faint cell boundaries that a grazing bump-mapped light amplifies into visible fake edges — exactly the artifact reported. Fixed by generating both grain octaves directly at full resolution and blurring with the same cumsum box-blur already used elsewhere in the script (cheap regardless of radius, no resampling grid to leave seams).

**The "still blurry" complaint, though, was never going to be solved by another texture pass — that was the actual point Ricky's frustration was flagging.** Parts 30-33 had already established, twice over with independent sources, that the soft hemisphere has no recoverable real detail at any resolution. Continuing to tune sharpening/contrast/grain around that hard limit was why four-plus passes all looked like "the same result" — they were all optimizing the edges of a constraint that doesn't move. The actual fix needed to be different in kind, not degree: dropped Pluto's ambient light from 0.4 to 0.12 in `renderPluto3D`. This isn't a texture trick standing in for missing data — Pluto has no atmosphere to scatter sunlight onto its unlit side, so a physically accurate render has a hard, dark terminator, not the softly-lit night side 0.4 was producing. The honest side effect is that this also naturally hides most of the one hemisphere with no real high-resolution data in shadow, rather than leaving it fully lit and visibly soft.

Verified with a Playwright reproduction of the terminator framing from Ricky's screenshot: the rectangle is gone (confirmed clean at both the same zoom and a wider framing), and the soft hemisphere now reads as a natural dark limb rather than a lit, visibly-soft patch — a genuinely different result, not a repeat of the last few passes. Bumped cache-busting to `?v=8`. Synced to `Aion/Frontend/Code/`.

---

### Current State

The bulk of Session 17 (everything through Part 34) is committed; Part 35 is ready to commit as a follow-up. All nine planets now have real-textured 3D renders with camera orbit controls, drag-to-reposition, and anisotropic filtering (Earth, Venus, Mercury, Mars, Jupiter, Saturn, Uranus, Neptune, Pluto) — the procedural Canvas 2D renderer is no longer used for any planet in this project. Pluto went through the deepest debugging arc of the session (Parts 24-34): wrong planet → real NASA data but upscaled → real resolution but still texel-limited up close → stale cache → missing anisotropic filtering (a real, separate fix that applies to every planet) → directional smear genuinely baked into the New Horizons source data, fixed by switching to a better-composited true-color source (Askaniy's 25K map) → sharpening/contrast tuning plus a bounded idle sway once continuous rotation was found to reliably carry the texture's one genuinely low-res band in front of the camera → easing back a sharpening/bumpScale combination that overshot into a fake "quilted" look at close zoom → softening the hard seams between the texture's different real detail tiers → and finally, fixing a real bump-map-only artifact from that seam-blending grain, plus dropping ambient light to give Pluto a physically accurate hard terminator that hides its one genuinely low-data hemisphere in shadow rather than leaving it lit and soft. `planet.html` also now has a live "Texture Look" panel (Part 35) letting Ricky tune contrast/saturation/focus/edge-blend himself per planet, without another round-trip. Also included: the Saturn ring-spin fix, the Saturn real-photo composite, the Saturn contrast/saturation enhancement, `compose_saturn_texture.py`, `compose_uranus_texture.py`, `compose_pluto_texture.py` (now v3, grain generation fixed), and `download_textures.py`'s fixes (blocked headers, crashing print, Venus texture correction, and the Pluto/Eris fix).

---

### Part 35 — Live "Texture Look" Panel: Contrast / Saturation / Focus / Blend Edges

After the whole Pluto texture arc (Parts 24-34), Ricky asked for a real feature rather than another round-trip: a way to adjust contrast (and related look controls) himself, live, instead of asking for a Python re-composite + screenshot every time something needed tuning. Added a new always-visible panel to `planet.html` (top-right, styled to match the existing EDIT LAYOUT panel) with four sliders, applied live to whichever planet is currently open:

- **Contrast** (50-200%) and **Saturation** (50-200%) — applied via `canvas.style.filter`, the same CSS filter mechanism `loadEnhancedTexture()` already uses at load time, just live-adjustable on top of it.
- **Focus** (0-100, soften only) — also a CSS `blur()` filter. Deliberately one-directional: CSS has no real "sharpen" filter, and fabricating fake sharpness beyond what's actually captured in the texture would misrepresent real photo data, the same principle the whole Pluto arc stuck to.
- **Blend edges** (0-100) — a live, general-purpose analog to `compose_pluto_texture.py`'s offline per-pixel detail-tier blending (Part 33): a second canvas overlaid on the WebGL canvas, filled via `drawImage` from it, blurred, and masked with a radial gradient so only the sphere's limb softens while the center stays untouched. Throttled to a ~180ms redraw interval (not per-frame — a slowly swaying/spinning planet doesn't need 60fps here, and copying the WebGL canvas every animation frame would be real overhead for no visible benefit).

All four values persist per-planet-slug in `localStorage` (`soulinterface_planet3d_look_v1`), same pattern as the existing planet-drag position and layout-panel storage, with a RESET button restoring defaults.

Two real bugs surfaced and got fixed during verification, not just claimed working from reading the code:
1. **Blend edges did nothing at first.** Added `preserveDrawingBuffer:true` to all 9 planets' `WebGLRenderer` instantiations (all identical, one `replace_all`) on the theory that WebGL was clearing its buffer between frames before the throttled `drawImage` could read it. Verified via direct pixel sampling that this wasn't actually the cause — `drawImage` was capturing real content fine even before the fix — but it's a correctness improvement worth keeping regardless (capture timing shouldn't be allowed to race the render loop), so left it in.
2. **The actual bug**: the radial-gradient mask's percentages were relative to CSS `circle`'s default sizing (farthest-corner — for a 1280x720 viewport, ~735px from center), while the sphere's actual on-screen radius is only ~275px at default zoom. The softening transition landed almost entirely past the visible disk, in empty space, so no amount of turning the slider up showed anything. Fixed by switching to `closest-side` sizing, which anchors 100% to the nearest viewport edge (the vertical half-height on this landscape layout) — much closer to the sphere's actual footprint. Confirmed only after pixel-sampling both canvases directly (via Playwright `getImageData`) to isolate "is the capture wrong" from "is the CSS wrong" — the capture was fine throughout; the gradient math was the actual bug.

Verified all four sliders live (screenshots at default, contrast=200, focus=100, blend=100) and the reset button restoring `localStorage` to defaults. Synced to `Aion/Frontend/Code/`. Not added to `index.html`'s small orbiting overview — the tiny on-screen scale there makes fine look-adjustment much less useful, and Ricky's ask was framed around the close-up view where the Pluto tuning actually happened.
...
### Part 37 — The Real Fix for Parts 32/34: Separate Bump Map from Color Map

Ricky sent two more screenshots and, fairly, said he wasn't satisfied and it was hard to believe this couldn't be done better. The screenshots showed a distinct, regular "orange peel"/drywall-knockdown pattern across the soft/flat hemisphere — his exact words, "it's literally just the same texture as drywall." That description was the key: a *regular*, uniform repeating pattern is not what real photographic softness or even random grain should look like once lit — it's the signature of narrow-frequency synthetic noise being read as height.

Root cause, finally addressed properly instead of tuned around again: `renderPluto3D` and index.html's Pluto block have pointed `.map` and `.bumpMap` at the *same* texture file since v1. Every purely cosmetic, color-only addition made to that one file — sharpening (Part 32), a resampling grid's faint edges (Part 34), and now v3's own two-octave grain — also becomes physical relief once the bump map reads it as height. Three different-looking bugs, one shared cause, and three rounds of parameter tuning (sharpening percent, bumpScale, grain amplitude) that only ever reduced the symptom because the actual architecture was never fixed.

`compose_pluto_texture.py` v4 now outputs two files instead of one: `tex_pluto.jpg` (color map — keeps the full treatment: sharpening, seam-blending, grain) and `tex_pluto_bump.jpg` (bump map — seam-blending only, no sharpening, no grain). Nothing added to the color texture for cosmetic reasons can be misread as height again, regardless of how the color side gets tuned in the future — the two are now structurally independent. Updated `renderPluto3D` and index.html's Pluto block to load `tex_pluto_bump.jpg` as a separate texture for `.bumpMap` rather than reusing the color texture object. Raised `bumpScale` back up (0.012 → 0.022 in both files) now that the bump source has nothing fake left in it to amplify — real crater rims get better shadow definition without the risk that came with the shared file.

Verified two ways: direct inspection of the new `tex_pluto_bump.jpg` at the previously-affected flat region confirmed it's genuinely smooth with zero grain (unlike the old shared file). Then, live on the rendered sphere — orbit controls don't reliably land on a requested spot via scripted drag gestures, so rather than force a specific angle, swept through several orbit positions and cropped into the crisp-to-soft transition zone at the one that matched Ricky's screenshot composition: real crater detail holding up cleanly right up to the terminator, fading into a smooth natural shadow gradient with no repeating pattern anywhere. Synced texture files + both HTML files to `Aion/Frontend/Code/`.
...
- General lesson from Part 34, the most important one of the Pluto arc: once a data limitation has been confirmed twice over (independent sources, checked at full resolution), stop tuning texture-processing parameters around it — that just produces visually-identical results pass after pass, which reads (correctly) as not listening. Look for a fix that's different in kind: here, a lighting/physics fix (hard terminator) rather than another sharpening/grain/contrast dial. Also: any procedural noise that gets upsampled from a coarser grid (BICUBIC or otherwise) should be treated as a bump-map risk even if it's invisible in plain color — check it under raking light before calling it done, not just from the default camera angle.
